...
void check_view_boundaries(character *self) {
    ...
    if (self->x < screen_left) {
        ...
        // Zero out leftward horizontal velocity completely
        if (self->physics.vx < 0.0f) {
            self->physics.vx = 0.0f;
        }
        
        // FIX: Completely clear out sub-pixel forces and friction accumulators
        // If your physics system uses structural force steps (like acceleration friction),
        // reset them here to stop the engine from fighting the screen position on the next tick:
        self->physics.ax = 0.0f; // Reset horizontal acceleration forces if present
    }
    ...
    if (player_right_side > screen_right) {
        ...
        // Zero out rightward horizontal velocity completely
        if (self->physics.vx > 0.0f) {
            self->physics.vx = 0.0f;
        }

        // FIX: Clear out acceleration forces to match the left-side behavior
        self->physics.ax = 0.0f; // Reset horizontal acceleration forces if present
    }
}
