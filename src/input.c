#include "input.h"
#include <string.h>

void input_init(input_state *state) {
    if (!state) return;
    state->active_actions = ACTION_NONE;
    state->move_x = 0.0f;
    state->move_y = 0.0f;
}

void input_update(input_state *state, joypad_port_t port) {
    if (!state) return;
    
    joypad_inputs_t raw = joypad_get_inputs(port);
    joypad_buttons_t pressed = joypad_get_buttons(port);
    joypad_buttons_t held = joypad_get_buttons_held(port);
    joypad_buttons_t released = joypad_get_buttons_released(port);

    state->move_x = (float)raw.stick_x / 85.0f;
    state->move_y = (float)raw.stick_y / 85.0f;

    state->active_actions = ACTION_NONE;
    if (raw.btn.d_left || state->move_x < 0.0f) state->active_actions |= ACTION_MOVE_LEFT;
    if (raw.btn.d_right || state->move_x > 0.0f) state->active_actions |= ACTION_MOVE_RIGHT;
    if (pressed.a && !held.a) state->active_actions |= ACTION_JUMP;
    if (pressed.a) state->active_actions |= ACTION_JUMP_HELD;
    if (released.a) state->active_actions |= ACTION_JUMP_RELEASED;
    if (pressed.b && !held.b) state->active_actions |= ACTION_ATTACK;
    
    if (pressed.start) {
        state->active_actions |= ACTION_START;
    }
}